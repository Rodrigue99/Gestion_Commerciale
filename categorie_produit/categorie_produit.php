...
// Renvoie au tableau de bord si l'utilisateur n'a pas accès à categ produit
...
switch ($_SESSION['type_user']) {
    ...
    case "Super Administrateur":
        addlog("Cons-01-categorie_produit", "", $_SESSION["prenom_user"]." ".$_SESSION["nom_user"]);
        break;
    case "Administrateur":
        addlog("Cons-02-categorie_produit", "", $_SESSION["prenom_user"]." ".$_SESSION["nom_user"]);
        break;
    case "Editeur":
        addlog("Cons-03-categorie_produit", "", $_SESSION["prenom_user"]." ".$_SESSION["nom_user"]);
        break;
    case "Auteur":
        addlog("Cons-04-categorie_produit", "", $_SESSION["prenom_user"]." ".$_SESSION["nom_user"]);
        break;
    case "Lecteur":
        addlog("Cons-05-categorie_produit", "", $_SESSION["prenom_user"]." ".$_SESSION["nom_user"]);
        break;
}
...
<head>
    ...
    <title>Gestion commerciale - catégories de produit</title>
    <!-- plugins:css -->
...
<body>
                                ...
                                <div class="row grid-margin">
                                    <div class="col-12">
                                        <div class="alert alert-success" role="alert">
                                            La liste comporte les informations éssentielles sur toutes les catégories d'articles.
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="form-group" style="width:300px; float:right;">
                                <a href="categorie-produit-add.php" style="text-decoration: none; color: white; margin-left: 30px;"><button type="button" class="btn btn-warning">Nouvelle catégorie d'article</button></a>
                            </div>
                            <div class="row">
                                <div class="col-12">
                                    <div class="card-body">
                                        <form method="POST" action="../export/export-categorie-produit.php">
                                            <div class="form-group" style="width:300px; float:right;">
                                                <div class="input-group">
                                                    <select name="export_categorie_produit" class="custom-select" id="inputGroupSelect04">
                                                    ...
                                                    <div class="input-group-append">
                                                        <button name="btn_export_categorie_produit" class="btn btn-primary" type="submit">Exporter</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </form>
                                        <div class="table-responsive">
                                            <table id="categorie_produit_data" class="table table-striped table-bordered" cellspacing="0" style="width:100%;">
                                                <thead>
                                                    <tr>
    ...
    <div id="categorie_produitModal_view" class="modal fade">
        <div class="modal-dialog">
            <form method="post" id="categorie_produit_form_view">
    ...
    <script type="text/javascript">
        ...
        var categorie_produitdataTable = $('#categorie_produit_data').DataTable({
            "processing": true,
            "serverSide": true,
            "order": [],
            "ajax": {
                url: "categorie_produit_fetch.php", // modifiable
                type: "POST"
            },
            "columnDefs": [{
        ...
        $(document).on('click', '.view', function() {
            var id_categorie_produit_view = $(this).attr("id");
            var btn_action_view = 'consulter';
            $.ajax({
                url: "categorie_produit_fetch_action.php",
                method: "POST",
                data: {
                    id_categorie_produit_view: id_categorie_produit_view,
                    btn_action_view: btn_action_view
                },
                dataType: "json",
                success: function(data) {
                    $('#categorie_produitModal_view').modal('show'); // affiche le modal

                    $('.modal-title_view').text(" Détails sur une catégorie d'articles");
                    $('#categorie_produits').html(data);
        ...
        $(document).on('click', '.remove', function() {
            var id_categorie_produit = $(this).attr('id');
                    ...
                    if (willDelete) {
                        ...
                        $.ajax({
...

                            {
                                ...
                                categorie_produitdataTable.ajax.reload();
                            }
                        });
                        swal('Effectué', 'Element supprimé', 'success');
                        categorie_produitdataTable.ajax.reload();
